...
func BuildStart(c *gin.Context) {
	...
	// build
	// add project name to local url
	vec := strings.Split(p.RepoUrl, "/")
	proj := strings.Split(vec[len(vec)-1], ".")[0]

	workSpace := gostring.JoinStrings(syncd.App.LocalTmpSpace, "/", gostring.Int2Str(build.ApplyId), "/", proj)
